...
void lcd_init(void)
{
 ...
 LCD_STROBE();
 send(0x28);	//4 bit mode
 send(0x08);	//display off - curser off
 send(0x01);	//clear disp
 send(0x06);	//shift curser to right
 send(0x80);	//begin to 1st line
 send(0x0c);	//display on - curser off
}
